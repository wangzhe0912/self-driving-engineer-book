...
对于自动驾驶车辆和先进的驾驶员辅助系统来说，对道路现场的识别是一个重要的组成部分，并且对学术界和汽车公司来说，这一点也正在变得越来越重要。本文提出了一种学习语义感知转换的方法，该转换将仪表盘摄像头视图中的检测映射到更广泛的场景鸟瞰图上。为此，从汽车仪表板和鸟瞰图上采集了一个巨大的合成数据集，其中包含1M多帧，并自动标注。然后训练一个深网络，使探测从第一视图到第二视图进行转换。我们证明了我们的模型对多个基线的有效性，并观察到，尽管只训练了合成基线，但它能够在真实数据上进行泛化。
